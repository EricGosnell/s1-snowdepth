...
def get_shap(df_nonan, model):
    ...
    df['snowclass'] = df['snowclass_1.0'] + df['snowclass_3.0'] + df['snowclass_5.0'] + df['snowclass_6.0'] + df[
        'snowclass_7.0'] + df['snowclass_2.0'] + df['snowclass_4.0']
    df = df.drop(
        columns=['snowclass_1.0', 'snowclass_3.0', 'snowclass_5.0', 'snowclass_6.0', 'snowclass_7.0', 'snowclass_2.0',
                 'snowclass_4.0'])
    df = df.rename(
        columns={'elevation': 'elevation_shap', 'slope': 'slope_shap', 'aspect': 'aspect_shap', 'fcf': 'fcf_shap', \
...
def prep_data(date, orbit, static_var, cfg: Config):
    ...
    static_crop = static_var.sel(lat=slice(lat_crop.max(), lat_crop.min()),
                                 lon=slice(lon_crop.min(), lon_crop.max()))  # crop static vars to same area
    ...
    df_x = pd.DataFrame(
        {'vv_scaled': all_var.vv_scaled.values.flatten(), 'cr_scaled': all_var.cr_scaled.values.flatten(),
         'lia': all_var.lia.values.flatten(), \
         'elevation': all_var.dem.values.flatten(), 'slope': all_var.slope.values.flatten(),
         'aspect': all_var.aspect.values.flatten(), \
         'fcf': all_var.forest.values.flatten(), 'tpi': all_var.TPI.values.flatten(),
         'sc_percum': all_var.sc_percum.values.flatten(), \
