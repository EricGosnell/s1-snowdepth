...
## Installation
For a step-by-step guide on how to install the package and run the model on sample data, see the Example section below.

This package is currently available on TestPyPi. 
First, create a virtual environment to install the package into:
`python -m venv .s1_snow && source .s1_snow/bin/activate` 
...
At this point, the virtual environment created beforehand is no longer needed, and you can optionally delete it with `rm -rf .s1-snow`.
...
Downloading the sample data using the command above and running the model on that data provided a method for quickly testing that everything is working before moving on to downloading and processing new data which may take a long time.
...
`s1-snowpepth create-s1-mosaic --date YYYYMMDD --orbit OOO --bbox min_lon,min_lat,max_lon,max_lat`. This will download the appropriate sentinel-1 data if available, and process it on ASF's HPC platform.
...
In addition to the snow cover percentage for each date, we must also calculate the cumulative snow cover percentage for each pixel from the given snow year, starting Aug 1.
Due to the amount of data that is required for this, running it for the first time can take a while however each subsequent run will be relatively quick.
The command to create the snow cover input is `s1-snowdepth create-snowcover YYYYMMDD` is based on the extent set by GEE_SEARCH_BBOX set in your .env.

## Example Set Up
Here is a step-by-step guide on how to install the package along with sample data to run the model and ensure everything works.
You are required to have python and conda already installed.

1. `mkdir s1-snowdepth && cd s1-snowdepth`
2. `python -m venv .s1-snow && source .s1-snow/bin/activate`
3. `pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ s1-snowdepth`
4. `s1-snowdepth env-create`
5. `deactivate`
6. `conda activate s1-snowdepth`
7. `rm -rf .s1-snow`
8. `s1-snowdepth init`
9. `s1-snowdepth download-model`
10. `s1-snowdepth download-static-vars`
11. `s1-snowdepth download-sample-data`
12. Set the following values in the .env file that was just created. It is recommended to use absolute filepaths instead of the relative ones shown below.
    - OUTPUT_DIR=./output/
    - S1_MOSAIC_DIR=./s1_mosaic/
    - S1_SCALING_DIR=./s1_scaling/
    - SNOW_COVER_DIR=./snow_cover/
    - STATIC_VAR_PATH=./static_vars/all_static_var.nc
    - SNOW_CLASS_PATH=./static_vars/snowclass.nc
    - LANDCOVER_PATH=./static_vars/landcover_1.nc
    - GRID_PATH=./static_vars/grid_lowres.nc
    - GLACIER_PATH=./static_vars/glacier_raster.nc
    - MODEL_PATH=.
13. `s1-snowdepth run`